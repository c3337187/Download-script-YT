...
def set_download_paths(base_path: str) -> None:
    """Обновляет глобальные пути загрузки и создаёт папки."""
    global DOWNLOADS_FOLDER, VIDEOS_FOLDER, PLAYLIST_FOLDER, PICTURES_FOLDER
    DOWNLOADS_FOLDER = base_path
    VIDEOS_FOLDER = os.path.join(DOWNLOADS_FOLDER, 'Videos')
    PLAYLIST_FOLDER = os.path.join(VIDEOS_FOLDER, 'Playlist Videos')
    PICTURES_FOLDER = os.path.join(DOWNLOADS_FOLDER, 'Pictures')
...
set_download_paths(DEFAULT_CONFIG['downloads_path'])
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                if 'downloads_path' in data:
                    data['downloads_path'] = os.path.abspath(data['downloads_path'])
...
def main() -> None:
    ...
    config = load_config()
    set_download_paths(config.get('downloads_path', DEFAULT_CONFIG['downloads_path']))
    ...
    def change_folder(icon, item):
        try:
            ...
            if path:
                ...
                save_config(config)
                set_download_paths(path)
                icon.notify('Готово', f'Путь изменён: {path}')
        except Exception as e:
            logging.error('Не удалось изменить папку загрузки: %s', e)
    ...
    menu = pystray.Menu(
        ...
        pystray.MenuItem('Изменить папку загрузки', change_folder),
